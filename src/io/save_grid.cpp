...
void save_grid_node(const std::string& prefix, const Grid& grid)
{
    ...
    std::string   f_name = create_out_file_name(prefix, "grd_nodes_", PIC::Config::current_time_step());
    std::ofstream ofsg(f_name.c_str());
    ofsg << "x\ty\tz\tNP\tUP\tUPx\tUPy\tUPz\tB\tBx\tBy\tBz\tE\tEx\tEy\tEz\tTe\n";
    ...
    for (index_t i = 0; i != grid.size_x(); ++i)
        for (index_t j = 0; j != grid.size_y(); ++j)
            for (index_t k = 0; k != grid.size_z(); ++k) {
                ...
                ofsg << i << "\t" << j << "\t" << k << "\t" << c.NP << "\t" << UP << "\t" << c.UP.x << "\t" << c.UP.y
                     << "\t" << c.UP.z << "\t" << B << "\t" << c.B.x << "\t" << c.B.y << "\t" << c.B.z << "\t" << E
                     << "\t" << c.E.x << "\t" << c.E.y << "\t" << c.E.z << "\t" << c.Te << '\n';
            }
}
...
{
    for (index_t k = 0; k != grid.size_z(); ++k) {
        std::string   f_name = fmt::format("gridz_{}_{}.dat", k, PIC::Config::current_time_step());
        std::ofstream ofsg(f_name.c_str());
        ofsg << "i\tj\tk\tNP\tUP\tUPx\tUPy\tUPz\tB\tBx\tBy\tBz\tE\tEx\tEy\tEz\tTe\n";
