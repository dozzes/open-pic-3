...
{
    template <class U> friend bool operator==(const Vector3D<U>& v1, const Vector3D<U>& v2);
  ...
  public:
    ...
    template <typename R> explicit Vector3D(const Vector3D<R>& other);
...
template <class T> inline bool operator==(const Vector3D<T>& v1, const Vector3D<T>& v2)
{
...
template <class T> inline bool is_finite(const Vector3D<T>& v)
{
